...
public class DBOpenHelperImpl extends SQLiteOpenHelper implements DBOpenHelper  {
	...
	/* (non-Javadoc)
	 * @see com.dreamteam.lookme.db.DBOpenHelper#saveOrUpdateContact(com.dreamteam.lookme.db.Contact)
	 */
	@Override
	public Profile saveOrUpdateProfile(Profile profile) throws Exception
	{
		Profile oldContact = getProfile(profile.getId());
			...
			}else
			{
				...
				contentValues.put(TABLE_PROFILES_COLUMN_NICKNAME, profile.getNickname());
				contentValues.put(TABLE_PROFILES_COLUMN_IMAGE, profile.getImage());
				database.update(TABLE_PROFILES, contentValues, "id", new String[]{"primary_id"});
			}
	...
	public List<Profile> getProfiles() throws Exception
	{
		...
		if(cursor.moveToFirst())
		{
			do{
				...
				tempProfile.setId(cursor.getInt(cursor.getColumnIndex(TABLE_PROFILES_COLUMN_ID)));
	...
	public void deleteProfile(long profileID) throws Exception
	{
		Cursor cursor = null;
		try{
		cursor = database.rawQuery("DELETE FROM " + TABLE_PROFILES+ " WHERE ID=?",new String[] {""+profileID});
	...
	public Profile getProfile(long contactID) throws Exception
	{
		...
		try{
			Profile tempProfile=new Profile();
		cursor = database.rawQuery("SELECT "+TABLE_PROFILES_COLUMN_ID + ", "+ TABLE_PROFILES_COLUMN_NAME +
				", "+ TABLE_PROFILES_COLUMN_SURNAME +", "+ TABLE_PROFILES_COLUMN_NICKNAME +", "+ TABLE_PROFILES_COLUMN_IMAGE +
				 " FROM " + TABLE_PROFILES + " WHERE ID=?",new String[] {""+contactID});
